...
function Reports({ user, role, appId }) {
    ...
    useEffect(() => {
      ...
      const unsubVar = onSnapshot(query(collection(db, 'artifacts', appId, 'public', 'data', 'variance_reports'), orderBy('timestamp', 'desc'), limit(50)), (s) => setVarianceReports(s.docs.map(d => ({id:d.id, ...d.data()}))));
      
      return () => { unsubLogs(); unsubIng(); unsubMenu(); unsubVar(); };
    }, [appId]);
  
    // --- New Logic: Calculate Advanced Insights ---
    useEffect(() => {
        if (ingredients.length === 0 || varianceReports.length === 0) return;
  
        // 1. Calculate Total Money Lost (Variance * Current Cost)
        let totalLoss = 0;
        varianceReports.forEach(rep => {
            rep.items?.forEach(item => {
                // Only look at negative variance (Loss)
                if (item.variance < 0) {
                    // Find the ingredient to get its cost
                    // Note: This matches by Name because reports snapshot the name. 
                    const ing = ingredients.find(i => i.name === item.name);
                    if (ing && ing.cost) {
                        totalLoss += (Math.abs(item.variance) * ing.cost);
                    }
                }
            });
        });
        ...
        // 2. Find Most Expensive Dish to Produce
        let maxCost = 0;
        let expDish = null;
    ...
    const generateOrderSummary = async () => {
      ...
      const systemPrompt = "You are a senior supply chain consultant. Respond using clear paragraphs.";
    ...
    const handleShare = async () => {
      ...
      Object.entries(groupedOrders).forEach(([supplier, items]) => {
          ...
          items.forEach(ing => {
              const deficit = (ing.minStock || 0) - (ing.currentStock || 0);
              text += `- ${ing.name}: ${Math.round(deficit * 100)/100} ${ing.unit}\n`;
          });
          text += `\n`;
      });
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                  ...
                  <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm relative overflow-hidden group">
                      <div className="relative z-10">
                          ...
                          <h3 className="text-3xl font-black font-mono text-slate-900">${wasteValue.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}</h3>
                          <p className="text-xs text-slate-400 mt-2 font-medium">Money lost to shortages (Historic)</p>
                      </div>
                  ...
                  <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm">
                      ...
                      {mostExpensiveDish ? (
                          <>
                              ...
                              <span className="bg-red-50 text-red-900 px-2 py-1 rounded text-xs font-bold border border-red-100">${mostExpensiveDish.cost.toFixed(2)} to make</span>
                          </>
...
  const handleFileUpload = (e) => { 
    setStep('processing'); // Transition quickly to hide potential white flash
    ...
    reader.onload = (evt) => {
        try {
            ...
            alert("Error processing CSV file. Please check format.");
            setStep('upload');
        }
    }; 
...
  const processCSV = (text) => { 
    ...
    // Find header line to start reading data
    for (let i = 0; i < lines.length; i++) { 
        if (lines[i].toLowerCase().includes('name') && lines[i].toLowerCase().includes('total')) { 
            startIdx = i + 1; 
            break; 
        } 
    } 
    ...
    for (let i = startIdx; i < lines.length; i++) { 
        ...
        // Basic attempt to parse fields, accommodating commas in quotes
        const parts = line.match(/(".*?"|[^",\s]+)(?=\s*,|\s*$)/g)?.map(p => p.replace(/^"|"$/g, '').trim()); 
        
        if (parts && parts.length >= 2) { 
            let qty = parseInt(parts[0]); 
            let name = parts[1]; 

            // Basic attempt to correct ordering if QTY is not first
            if (isNaN(qty)) { 
                qty = parseInt(parts[1]); 
                name = parts[0]; 
            } 
            
            if (!isNaN(qty) && name) data.push({ qty, name }); 
        } 
    } 
    
    const matched = data.map(row => { 
        const item = localMenuItems.find(m => m.name.toLowerCase() === row.name.toLowerCase()); 
        return { 
            ...row, 
            found: !!item, 
            itemData: item || null 
...
 
  // **UPDATED:** Logic to submit to Admin Review instead of direct stock deduction
  const applyStockChanges = async () => {	
    setStep('processing');	
   	
    if (matchedData.length === 0) {
        alert("No valid sales data matched. Please check your CSV or Menu items.");
        setStep('upload');
        return;
    }
...
       	
        // 1. Update Sales Count (soldCount) - This is a data metric, safe to update directly
...
        // 2. Prepare Stock DEDUCTION for Admin Review (reportItems)
...
               	
                // Deduction is stored as 'change' (negative value) in the pending report
                reportItems.push({ 
                    id: ing.ingredientId, 
                    name: `${row.name} (Ing: ${ing.ingredientName})`, 
                    change: -deductQty, // The actual deduction amount
                    unit: ingData?.unit || 'units',
                    currentSystemStock: ingData?.currentStock || 0, // snapshot of current stock
                    countedStock: 0 // Placeholder
                });
            }	
...
    // 3. Create PENDING Sales Deduction Report in stockCounts collection
    const reportDocRef = doc(collection(db, 'artifacts', appId, 'public', 'data', 'stockCounts'));
    await setDoc(reportDocRef, {
        submittedBy: user.uid,
        timestamp: serverTimestamp(),
        status: 'pending_sales_deduction', // NEW STATUS for sales
...
    setStep('upload');	
    setMatchedData([]);	
    alert("Sales submitted for admin approval. Stock will be adjusted after review.");	
  };
