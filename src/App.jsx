...
async function callGemini(systemInstruction, userQuery, isJson = false, schema = {}) {
    const payload = {
        contents: [{ parts: [{ text: userQuery }] }],
        config: {
            ...(isJson ? { responseMimeType: "application/json" } : {}),
            ...(Object.keys(schema).length > 0 ? { responseSchema: schema } : {}),
        },
    ...
    while (attempts < maxAttempts) {
        try {
            ...
            if (isJson) {
                try {
                    // LLM responses sometimes include markdown code blocks for JSON, try to strip them
...
const PinModal = ({ isOpen, onClose, onSuccess, title = "Enter Admin PIN", appId }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (pin === correctPin) {
        onSuccess();
        // NOTE: Do not close modal here if onSuccess navigates away
        // onClose();
        setPin('');
      } else {
...
// Async Log Helper
async function addLog(db, appId, uid, message) { 
...
export default function InventoryApp() {
    ...
    const mobileNavItems = role === 'admin' 
      ? ['dashboard', 'ingredients', 'stock_value', 'receive', 'menu', 'upload', 'stocktake', 'reports']
      : ['dashboard', 'receive', 'stocktake', 'reports']; // Staff Views
      ...
      <div className="flex h-[100dvh] bg-slate-50 text-slate-800 font-sans overflow-hidden">
        ...
        <aside className="w-72 bg-white border-r border-slate-200 flex-shrink-0 flex flex-col shadow-2xl z-20 hidden md:flex">
          ...
          <nav className="flex-1 overflow-y-auto py-6 space-y-1 px-3">
            ...
            {role === 'admin' && <NavItem icon={Package} label="Ingredients & Costs" view="ingredients" current={view} setView={setView} />}
            {role === 'admin' && <NavItem icon={DollarSign} label="Stock Value" view="stock_value" current={view} setView={setView} />}
            <NavItem icon={Truck} label="Receive Stock" view="receive" current={view} setView={setView} />
            
            {/* Kitchen Section - ADMIN ONLY */}
            {role === 'admin' && (
              <>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 rounded-3xl p-6 md:p-8 text-white shadow-xl relative overflow-hidden">
        ...
        <h3 className="text-4xl md:text-5xl font-black tracking-tight font-mono">Rs {grandTotal.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</h3>
      </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {sortedIngredients.map(ing => (
                <tr key={ing.id} className="hover:bg-slate-50/50 transition">
                  ...
                  <td className="p-4 text-right text-slate-500 font-mono text-xs">Rs {Number(ing.cost).toFixed(2)}</td>
                  <td className={`p-4 text-right font-mono font-black ${ing.totalValue > 100 ? 'text-slate-900' : 'text-slate-400'}`}>Rs {ing.totalValue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-2 gap-6">
            ...
            <div>
              ...
              <div className="relative"><span className="absolute left-4 top-4 text-slate-400 font-bold">Rs</span><input type="number" step="0.0001" className="w-full border-2 border-slate-100 bg-slate-50 p-4 pl-10 rounded-xl font-mono font-bold" value={formData.cost} onChange={e => setFormData({...formData, cost: e.target.value})} placeholder="0.00" /></div>
            </div>
            <div>
            ...
            {viewMode === 'grid' ? (
                ...
                {filteredIngredients.map(ing => (
                    ...
                    <div className="flex justify-between items-center pl-2 pt-4 border-t border-slate-50">
                        <span className="text-xs font-bold text-slate-400">Cost: <span className="text-slate-700 font-mono">Rs {Number(ing.cost || 0).toFixed(2)}</span>/{ing.unit}</span>
                ...
                <div className="overflow-x-auto">
                    ...
                    <tbody className="divide-y divide-slate-100">
                        ...
                        <tr key={ing.id} className="hover:bg-slate-50/50">
                            ...
                            <td className="p-4 text-right font-mono">Rs {Number(ing.cost).toFixed(2)}</td>
...
function MenuManager({ user, role, appId }) {
  ...
  const optimizeRecipe = async () => {
    ...
    const prompt = `Analyze the recipe for "${formData.name}" (Total Cost: Rs ${currentRecipeCost.toFixed(2)}). Identify the highest-costing ingredient(s) and provide two realistic, specific cost-saving suggestions for a restaurant kitchen. The current ingredients are: ${JSON.stringify(recipeDetails)}.`;
    ...
    return (
      <div className="bg-white p-4 md:p-8 rounded-3xl shadow-2xl max-w-3xl mx-auto border border-slate-100 animate-in zoom-in-95">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
           ...
           {formData.type === 'recipe' && (
             <div className="space-y-4 animate-in fade-in slide-in-from-top-2">
               <div className="bg-red-50 p-4 md:p-6 rounded-2xl border border-red-100">
                  ...
                  <ul className="space-y-2 mb-4">{formData.recipe.map((r, i) => { const ing = ingredients.find(inG => inG.id === r.ingredientId); const cost = ing ? (ing.cost * r.qty).toFixed(2) : '0.00'; return (<li key={i} className="flex justify-between items-center text-sm bg-white p-3 rounded-xl border border-red-100 shadow-sm"><span className="font-bold text-slate-700">{r.qty} {ing?.unit} {r.ingredientName}</span><div className="flex items-center gap-3"><span className="text-slate-400 font-mono text-xs font-bold">Rs {cost}</span><button type="button" onClick={() => setFormData(prev => ({...prev, recipe: prev.recipe.filter((_, idx) => idx !== i)}))} className="text-red-400 hover:text-red-600"><Trash2 size={16}/></button></div></li>); })}</ul>
                  <div className="text-right border-t border-red-200 pt-3 flex justify-between items-center">
                    ...
                    <div>
                        ...
                        <span className="font-mono font-bold text-slate-900 text-lg">Rs {currentRecipeCost.toFixed(2)}</span>
                    </div>
                  </div>
               </div>
           ...
           {formData.type !== 'non_stock' && (<div><label className="block text-xs font-black text-slate-400 mb-2 uppercase tracking-wide">Other Costs</label><div className="relative"><span className="absolute left-4 top-4 text-slate-400 font-bold">Rs</span><input type="number" step="0.01" className="w-full border-2 border-slate-100 bg-slate-50 p-4 pl-10 rounded-xl font-mono font-bold text-slate-800" value={formData.otherCost} onChange={e => setFormData({...formData, otherCost: e.target.value})} placeholder="0.00" /></div></div>)}
           {formData.type !== 'non_stock' && (<div className="bg-slate-900 text-white p-6 rounded-2xl flex justify-between items-center shadow-lg"><span className="font-black text-sm uppercase tracking-wide text-slate-400">Total Cost</span><span className="font-mono text-3xl font-black text-white">Rs {(formData.type === 'recipe' ? (currentRecipeCost + (parseFloat(formData.otherCost) || 0)) : ((ingredients.find(i => i.id === formData.linkedIngredientId)?.cost || 0) + (parseFloat(formData.otherCost) || 0))).toFixed(2)}</span></div>)}
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredItems.map(item => { const totalCost = calculateTotalCost(item); return (
                  <tr key={item.id} className="hover:bg-slate-50/50 transition">
                    ...
                    <td className="p-6"><span className="text-[10px] font-black uppercase tracking-wider bg-slate-100 text-slate-50 px-2 py-1 rounded">{item.category || 'Mains'}</span></td>
                    ...
                    <td className="p-6 text-sm text-slate-500">
                      {item.type !== 'non_stock' && <div className="flex flex-col gap-1"><span className="text-xs font-medium">Ingredients: <span className="font-mono text-slate-700 font-bold">Rs {(totalCost - (item.otherCost || 0)).toFixed(2)}</span></span>{item.otherCost > 0 && <span className="text-xs font-medium">Other: <span className="font-mono text-slate-700 font-bold">Rs {Number(item.otherCost).toFixed(2)}</span></span>}</div>}
                      {item.type === 'non_stock' && <span className="text-slate-300">-</span>}
                    </td>
                    <td className="p-6 text-right font-mono font-black text-slate-900 text-lg">{item.type !== 'non_stock' ? `Rs ${totalCost.toFixed(2)}` : '-'}</td>
          ...
          {filteredItems.map(item => { const totalCost = calculateTotalCost(item); return (
            <div key={item.id} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col justify-between">
                ...
                <div className="mt-4 pt-4 border-t border-slate-50 flex justify-between items-end">
                    <span className="font-mono font-black text-2xl text-slate-900">{item.type !== 'non_stock' ? `Rs ${totalCost.toFixed(2)}` : '-'}</span>
...
function CSVUploader({ user, role, appId }) {
  ...
  useEffect(() => { 
    ...
    const unsubMenu = onSnapshot(collection(db, 'artifacts', appId, 'public', 'data', 'menu_items'), (snap) => {
        setLocalMenuItems(snap.docs.map(d => ({id:d.id, ...d.data()})));
        setLoading(false);
    });
    const unsubIng = onSnapshot(collection(db, 'artifacts', appId, 'public', 'data', 'ingredients'), (snap) => setLocalIngredients(snap.docs.map(d => ({id:d.id, ...d.data()}))));
    return () => { unsubMenu(); unsubIng(); };
  }, [appId]);
  ...
  const handleFileUpload = (e) => { 
    ...
    reader.onload = (evt) => {
        try {
            ...
            alert("Error processing CSV. Please ensure format is correct.");
            setStep('upload');
        }
    }; 
    ...
    for (const row of matchedData) {     
        ...
        if (row.itemData.type === 'recipe') {     
            ...
            for (const ing of row.itemData.recipe) {     
                ...
                reportItems.push({ 
                    id: ing.ingredientId, 
                    name: `${row.name} (Ing: ${ing.ingredientName})`, 
                    change: -deductQty, 
  ...
  return (
    <div className="max-w-4xl mx-auto">
        <h2 className="text-3xl font-black text-slate-900 mb-8">Upload Daily Sales</h2>
        ...
        {step === 'upload' && (
            <div className="bg-white p-12 rounded-3xl shadow-sm border-2 border-dashed border-slate-200 text-center hover:border-red-900 transition-colors group">
                ...
                <p className="text-xl font-bold text-slate-800 mb-2">Upload POS CSV</p>
                <label className="inline-block cursor-pointer">
                    <span className="bg-slate-900 text-white px-8 py-4 rounded-2xl font-bold hover:bg-slate-800 transition shadow-lg hover:shadow-xl transform hover:-translate-y-1 block">Select CSV File</span>
        ...
        {step === 'processing' && (
            <div className="flex h-48 items-center justify-center bg-white rounded-3xl shadow-sm border border-slate-200">
                <div className="text-center">
                    <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-red-900 mx-auto mb-3"></div>
        ...
        {step === 'preview' && (
            <div className="bg-white rounded-2xl shadow-lg overflow-hidden border border-slate-200">
                <div className="p-6 border-b border-slate-100 bg-slate-50 flex flex-wrap justify-between items-center gap-3">
                    <h3 className="font-bold text-slate-800 text-lg">Preview & Match</h3>
                    <div className="space-x-3">
                        <button onClick={() => setStep('upload')} className="px-5 py-2 text-sm font-bold text-slate-500 hover:text-slate-800">Cancel</button>
                        <button onClick={applyStockChanges} className="px-6 py-2 text-sm bg-green-600 text-white rounded-xl font-bold hover:bg-green-700 shadow-md">Confirm & Submit</button>
                    </div>
                </div>
                <div className="overflow-x-auto">
